...
class Hospital:
        ...
        if self.Nameoftablets.get() == "" or self.ref.get() == "":
            ...
            try:
                conn = mysql.connector.connect(
                    ...
                    password='Pyadav001',      # replace with your actual MySQL password
                    database='mydata',
                    port=3306
                )
    ...
    def fatch_data(self):
        conn=mysql.connector.connect(host="localhost",username="root",password="Pyadav001",database="mydata")
        my_cursor=conn.cursor()
    ...
    def idelete(self):
        try:
            ...
            my_cursor = conn.cursor()
            # Confirm the column name for reference number is 'ref' or 'Reference_No'
            # Earlier you used 'ref' in insert, 'Reference_No' in delete
            # Use 'ref' if that matches your table column name or adjust accordingly
            query = "DELETE FROM hospital WHERE ref = %s"
            value = (self.ref.get(),)
            my_cursor.execute(query, value)
            conn.commit()
            if my_cursor.rowcount == 0:
                messagebox.showwarning("Delete", "No record found with this Reference No.")
            else:
                messagebox.showinfo("Delete", "Patient has been deleted successfully.")
                self.fatch_data()  # Refresh table to reflect deletion
